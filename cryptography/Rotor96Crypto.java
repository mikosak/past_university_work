...
public class Rotor96Crypto {
	...
	public static void main(String[] args) {
		String ciphertext1 = "jc4DFCdyO/Wx6Tek/j[.c<X&57o8E3$IWw\\-LN@Tr}L}(0$]k9N_2kPuG5bx)q^1!#bFqBdMv?`Stl^yNEwI@U_Q}:y&}-,U((48ma\\62!$:~&r{lSL7`mdv08p WIbLbId,ZlO29H~vR3zy9Ah:AxiAym{\"ZWa|:KSP/=;(Od:^vsya7J\\_gqI5I[osj)$<MX;jz+s$:>cEY!8U9Xxo%FldV~\"Lm6%-L,ETmSd(pvP_sBIpGT.Rdiuk~=GIQNe4+7WvM`rhrxk";
		String ciphertext2 = "F{qQ,Fg*Ve{U}!1;E]1C#3[mAzJNiI6@8Y/AA&\\Cy?[A1~l[i/'BvZz.P_dV2<;&]n@}pRYinw,.\"b/A3G& plR_DL~|uCur{'^b4`sa(<Amn0WB)Ikv;9^oH7ywwFnk,`%A6hX*w6we?>:!m-+<#}?Bw3W'nfZ2vE\"<`p_07]xl])g\\y5c>=q/2_EIb93u_d-|aZ~fxHVsAu(B`Gs'0@:=ssh#,96`O7A=({s-5>elF<TLRq$tA[B=\\VMu#ls.Y<6Wf_07c \"yNn3vOGzG JJPb{cpdk63Xvfh;'IR+NyHis~rsM4K~ToSRfYfFO'i/`#>j}Awp6eIkY!LJBv YOBhOmy!*82}o#bR]L;^}h$ak7<FdR1-Qj_*\\Mb£\\lJL%u-ces*xyhsB?s~qGFNe>N/8{!";

        try (
            BufferedReader reader = new BufferedReader(new FileReader("passwords"));
            PrintWriter writer = new PrintWriter(new FileWriter("experiment_output"));
        ) {
            String key;
			String word;
            while ((key = reader.readLine()) != null) {
                ...
                String decrypted1 = encdec(DEC, key, ciphertext1);
				String decrypted2 = encdec(DEC, key, ciphertext2);
				BufferedReader english = new BufferedReader(new FileReader("words_trimmed.txt"));
				while ((word = english.readLine()) != null) {
					if (decrypted1.startsWith(word)) {
						writer.println("Key: " + key);
						writer.println("Decrypted Text 1: ");
						writer.println(decrypted1);
						writer.println("-------------------------------------------------");
					}
					if (decrypted2.startsWith(word)) {
						writer.println("Key: " + key);
						writer.println("Decrypted Text 2: ");
						writer.println(decrypted2);
						writer.println("-------------------------------------------------");
					}
				}
				english.close();
            }
            System.out.println("Decryption attempts complete. Results saved to 'experiment_output'.");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
