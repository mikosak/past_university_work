...
struct theTable
{
  private:
    ...
    std::mutex theTableMutex;

public:
    std::list<std::string>& operator[](const std::string& key) {
        std::lock_guard<std::mutex> lock(theTableMutex);
        return theTable[key];
    }

    std::unordered_map<std::string, std::list<std::string>>::iterator find(const std::string& key) {
        std::lock_guard<std::mutex> lock(theTableMutex);
        return theTable.find(key);
    }

    bool insert(const std::pair<std::string, std::list<std::string>>& entry) {
        std::lock_guard<std::mutex> lock(theTableMutex);
        if (theTable.find(entry.first) != theTable.end()){
          return false;
        }
        theTable.insert(entry);
        return true;
    }
...
struct workQ
{
  private:
    ...
    std::mutex workQMutex;

public:
    void push_back(const std::string& item) {
        std::lock_guard<std::mutex> lock(workQMutex);
        q.push_back(item);
    }
...
static void process(const char *file, std::list<std::string> *ll) {
  ...
  while (fgets(buf, sizeof(buf), fd) != NULL) {
    ...
    *q = '\0';
    ...
    if (table.find(name) != table.end()) { continue; }
...
static void printDependencies(std::unordered_set<std::string> *printed,
  ...
  while ( toProcess->size() > 0 ) {
    ...
    std::list<std::string> *ll = &table[name];
...
int main(int argc, char *argv[]) {
  ...
  auto processFilesFunction = [&]() -> void {
    while ( Q.size() > 0 ) {
      std::string filename = Q.pop_front();
      ...
      process(filename.c_str(), &table[filename]);
    }
  };
  // 1. look up CPATH in environment
  ...
  if (getenv("CRAWLER_THREADS") != NULL){
    char *strThreads = getenv("CRAWLER_THREADS");
    numThreads = atoi(strThreads);
  }
  // 4. for each file on the workQ
  std::vector<std::thread> threads;
  for (int i = 0; i < numThreads; ++i) {
        threads.emplace_back(std::thread(processFilesFunction));
    }
